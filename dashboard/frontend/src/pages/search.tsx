...
export function SearchPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
        ...
        <div className="flex gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-muted)]/50 py-2.5 pl-9 pr-3 text-sm outline-none focus:border-[var(--color-primary)] transition-colors"
            />
            {query && (
              <button onClick={() => { setQuery(""); setResults([]); setSearched(false) }} className="absolute right-3 top-1/2 -translate-y-1/2 text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)]">
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-lg bg-[var(--color-primary)] px-5 py-2.5 text-sm font-medium text-white hover:opacity-90 disabled:opacity-40 transition-opacity"
          >
            {loading ? (
        ...
        <div className="space-y-3">
          ...
          {results.map((r, i) => (
            ...
            >
              ...
              {r.snippet && (
                <p className="mt-2 text-sm text-[var(--color-muted-foreground)] line-clamp-2">{r.snippet}</p>
              )}
              {r.link && (
                <a
                  href={r.link}
      ...
      {!searched && (
        <div className="flex h-40 items-center justify-center rounded-xl border border-dashed border-[var(--color-border)] bg-[var(--color-card)]/50">
          <div className="text-center">
            <Search className="mx-auto h-8 w-8 text-[var(--color-muted-foreground)] mb-2" />
            <p className="text-sm text-[var(--color-muted-foreground)]">Type a query and press Enter to search</p>
          </div>
        </div>
      )}
    </motion.div>
