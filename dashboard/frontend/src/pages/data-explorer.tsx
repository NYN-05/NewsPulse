...
export function DataExplorerPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      <div className="flex flex-wrap items-center gap-3">
        <div className="relative flex-1 min-w-[200px] max-w-sm">
          ...
          <input
            ...
            className="w-full rounded-lg border border-(--color-border) bg-card py-2 pl-8 pr-3 text-sm outline-none focus:border-primary transition-colors"
          />
        </div>
        <select
          value={categoryFilter}
          onChange={(e) => setCategoryFilter(e.target.value)}
          className="rounded-lg border border-(--color-border) bg-card px-3 py-2 text-xs outline-none"
        >
        ...
        <button onClick={fetchData} className="rounded-lg border border-(--color-border) p-2 hover:bg-muted transition-colors" title="Refresh">
      ...
      {/* Table */}
      <div className="overflow-hidden rounded-xl border border-(--color-border) bg-card">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-(--color-border) bg-muted/30">
                ...
                <th className="p-3 text-left text-xs font-medium text-muted-foreground">Sentiment</th>
                <th className="p-3 text-left text-xs font-medium text-muted-foreground">Category</th>
                <th className="p-3 text-left text-xs font-medium text-muted-foreground">Lang</th>
                <th className="p-3 text-left text-xs font-medium text-muted-foreground">Virality</th>
            ...
            <tbody>
              {loading ? (
                <tr>
                  <td colSpan={7} className="p-8 text-center text-sm text-muted-foreground">
                    <span className="inline-flex items-center gap-2">
                      <span className="h-4 w-4 animate-spin rounded-full border-2 border-(--color-border) border-t-primary" />
                      Loading...
                    </span>
                  </td>
                </tr>
                ...
                filtered.map((r, i) => (
                  <tr key={i} className="border-b border-(--color-border) transition-colors hover:bg-muted/20">
                    ...
                    <td className="p-3 text-muted-foreground text-xs">{r.language || "—"}</td>
                    <td className="p-3 text-xs">{(r.virality_score ?? 0).toFixed(3)}</td>
                    <td className="p-3 text-xs text-muted-foreground">{r.published?.slice(0, 10)}</td>
                  </tr>
                ))
              )}
            </tbody>
